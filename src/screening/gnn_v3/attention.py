"""v3 交叉图注意力 — 双向 Cross-Attention，醛原子关注胺原子（反之亦然）。

输入: 醛原子嵌入 [N_ald, hidden_dim] + 胺原子嵌入 [N_amine, hidden_dim]
输出: 交叉注意力后的醛原子嵌入 [N_ald, hidden_dim] + 胺原子嵌入 [N_amine, hidden_dim]
"""
...
class CrossGraphAttention(nn.Module):
    ...
    Args:
        hidden_dim: 输入/输出维度
        num_heads: 多头注意力头数 (hidden_dim 必须被 num_heads 整除)
