...
def main():
    ...
    overview = [
        ...
        ["当前规则配置", "规则#1 总芳环≤5 [开启] | 规则#3 N杂环加分 β=0.05 [开启]"],
    ...
    _add_table(doc, ["规则", "状态", "说明"],
               [["#0 苯环必需", "启用", "无苯环单体排除"],
                ["#1 总芳环≤5", "启用", "苯环+杂芳环总数上限 (≤5), 过滤超大共轭体系"],
                ["#2 对称性", "增强", "CanonicalRankAtoms 全分子拓扑对称 (中心+镜像)"],
                ["#3 N 杂环加分", "启用", "含 N 杂芳环 (吡啶/嘧啶/三嗪等) → β=0.05 轻量加分"],
                ["#4 C2 对位", "增强", "单苯环→1,4对位; 多苯环→对称+允许非对位"],
                ["#4b 直链苯", "启用", "≥3 对位直链苯→软惩罚 (越长惩罚越大, 非直链不惩罚)"],
                ["#5 炔丙基醚", "启用", "醚+炔共存排除"],
                ["#6 取代基限卤素", "启用", "C2>4 / C3>5 取代限卤素 (非卤素取代须为反应基团)"],
                ["#7 官能团", "启用", "≥2 醛/胺基"]],
    ...
    methods = [
        ...
        ["加成系数", "C3-胺 ×1.15, C3-醛 ×1.10, N杂环 ×1.05 (池内排序生效)"],
        ["直链苯惩罚", "≥3 对位直链苯环 → max(0.4, 1−(n−3)×0.08) 惩罚因子"],
        ["硬规则 #1", "总芳环 (苯环+杂芳环) ≤5, 过滤超大共轭体系"],
